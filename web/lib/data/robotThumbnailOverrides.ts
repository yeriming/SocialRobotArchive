export const ROBOT_THUMBNAIL_OVERRIDES: Record<string, string[]> = {
  "ROBOT-0001": ["asset://paro-manual"],
  "ROBOT-0002": [
    "https://cdn.prod.website-files.com/68df6f147a002d319954fff7/68df6f147a002d3199550094_abi-.avif"
  ],
  "ROBOT-0005": [
    "https://site-static.ecovacs.com/upload/us/image/product/2026/05/06/022734_9248$lilmilo-white-920x920-1.png"
  ],
  "ROBOT-0006": ["https://store.enabot.com/cdn/shop/files/EBO_X.jpg?v=1771132178"],
  "ROBOT-0007": ["https://store.enabot.com/cdn/shop/files/EBO-Max-A_-_1.jpg?v=1778212831"],
  "ROBOT-0008": [
    "https://store.enabot.com/cdn/shop/files/Mini_Sport_-_3000x3000_8ffb6c03-bbaf-47a3-8e61-e48032d9e7bd.jpg?v=1779357919"
  ],
  "ROBOT-0011": ["asset://joobie-manual"],
  "ROBOT-0012": ["https://data.pophie.com/website/_astro/pophie-01.CoP9ueSJ_1mWnRq.webp"],
  "ROBOT-0013": ["https://elliq.com/cdn/shop/files/Frame_1321317507.png?v=1758024522"],
  "ROBOT-0014": [
    "https://cdn.shopify.com/s/files/1/0750/4170/2077/files/hero-dm-normal-banner-d-v1.webp?v=1785228678"
  ],
  "ROBOT-0017": ["asset://cocomo-manual"],
  "ROBOT-0018": ["asset://inu-manual"],
  "ROBOT-0019": ["asset://rumi-manual"],
  "ROBOT-0020": [
    "https://m5stack-doc.oss-cn-shenzhen.aliyuncs.com/1205/K151_stack_chan_main_pictures_01.webp"
  ],
  "ROBOT-0021": ["https://ui44.com/images/robots/magicdog.webp"],
  "ROBOT-0022": ["https://ui44.com/images/robots/anan-panda.webp"],
  "ROBOT-0023": [
    "https://assets.robotsguide.com/images/7p2whiua/production/e2dbd260ec6afdcde3383d4212f1451af4bff3c8-1200x630.jpg?w=1200&h=630"
  ],
  "ROBOT-0024": ["https://ui44.com/images/robots/miko-3.webp"],
  "ROBOT-0025": ["https://ui44.com/images/robots/miko-mini.png"],
  "ROBOT-0030": ["https://ui44.com/images/robots/reachy-mini.jpg"],
  "ROBOT-0031": ["asset://prime-t1-manual"],
  "ROBOT-0032": [
    "https://cdn.prod.website-files.com/6655b1a239c70c0be4c6e48d/6661b2b67ffc5bdb0d8e8d37_web%201%201.jpg"
  ],
  "ROBOT-0033": [
    "https://www.koibot.online/cdn/shop/files/koibot-rola-one-look2.jpg?v=1783691809&width=1100"
  ],
  "ROBOT-0034": ["https://ui44.com/images/robots/rhem.webp"],
  "ROBOT-0035": ["https://ui44.com/images/robots/fuzozo.webp"],
  "ROBOT-0036": ["asset://ballie-manual"],
  "ROBOT-0038": ["asset://aibo-manual"],
  "ROBOT-0039": [
    "https://www.switch-bot.com/cdn/shop/files/Noa-1_3x_f38f33c5-62fe-4252-be07-7a5914a13e0c_1200x1200.webp?v=1776222050"
  ],
  "ROBOT-0040": [
    "https://tombot.com/cdn/shop/files/IMG_4159-Tombot_2025_Selects-23-WE.jpg?v=1767504868"
  ],
  "ROBOT-0041": ["asset://sweekar-manual"],
  "ROBOT-0042": ["asset://tuya-aura-manual"],
  "ROBOT-0043": ["asset://alpha-mini-manual"],
  "ROBOT-0044": ["https://ui44.com/images/robots/kangbao-health-baby.webp"],
  "ROBOT-0045": ["https://i.ytimg.com/vi/k_1Paexlc_A/hqdefault.jpg"],
  "ROBOT-0047": ["https://ui44.com/images/robots/mirumi.webp"],
  "ROBOT-0049": ["asset://jibo-manual"],
  "ROBOT-0053": [
    "https://www.softbank.jp/-/media/sb/sjp/robot/set/data/img/shared/mainvisual/550x550/img-pepper-20180911/img-customer_en.png?rev=dccf369f56df4f81863171e653ff2f03&sc_lang=en&h=550&w=550&la=en&hash=EE7F3DCC2DD7CF273DAE9E8BFDD86F30"
  ]
};

export const ROBOT_THUMBNAIL_STRICT_IDS = new Set<string>([
  "ROBOT-0049", // Jibo
  "ROBOT-0047", // Mirumi
  "ROBOT-0044", // Kangbao (Health Baby)
  "ROBOT-0043", // Alpha Mini
  "ROBOT-0042", // Aura
  "ROBOT-0041", // Sweekar
  "ROBOT-0040", // Jennie
  "ROBOT-0036", // Ballie
  "ROBOT-0035", // Fuzozo
  "ROBOT-0034", // Rhem
  "ROBOT-0032", // David
  "ROBOT-0033", // KOIBOT ROLA Series
  "ROBOT-0031", // Prime T1
  "ROBOT-0030", // Reachy Mini
  "ROBOT-0025", // Miko Mini
  "ROBOT-0024", // Miko 3
  "ROBOT-0023", // Kuri
  "ROBOT-0022", // An'An Panda
  "ROBOT-0021", // MagicDog
  "ROBOT-0020", // StackChan
  "ROBOT-0018", // INU
  "ROBOT-0017", // Cocomo
  "ROBOT-0014", // Loona
  "ROBOT-0012", // Pophie
  "ROBOT-0005", // LilMilo
  "ROBOT-0002", // Abi
  "ROBOT-0001" // PARO
]);
